...
contract CrowdFunding {
    ...
    function createProject(string memory _title, string memory _description) public {
        ...
        //TODO: check for duplicate project titles -- no need
        ...
        //console.log("createProject: %s, %s, %s", projects[projectCounter].projectID, projects[projectCounter].projectTitle, projects[projectCounter].projectOwner);
    ...
    function participateToProject(uint256 _projectID) public payable {
        ...
        //console.log("participateToProject-project: %s, participant: %s, contribution: %s", _projectID, msg.sender, contributions[_projectID][msg.sender]);
    ...
    */
    ...
    function withdrawFunds(uint256 _projectID) public {
        ...
        require(address(this).balance >= prj.projectTotalFundingAmount, "Rug pull detected"); // :D
       
        (bool success,) = msg.sender.call{value: prj.projectTotalFundingAmount}("");        
        if(!success){
            revert("Withdrawal unsuccessfull");
        }else{            
            //console.log("withdrawFunds-project: %s, withdrawer: %s, amount: %s", _projectID, msg.sender, prj.projectTotalFundingAmount);
